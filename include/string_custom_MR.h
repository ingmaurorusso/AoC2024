...
auto& operator<<(std::ostream &os, const ReferSomething& rf) noexcept{
    return os << *rf;
}

template<typename T>
concept Cprintable1 = requires(std::ostream os, T t){os << t;};

namespace NString__MR{
    // forward declarations
    // An alternative may be to define operator<< for iterables, after the namespace.
...
namespace NString__MR{
    ...
    requires requires(std::ostream ostream, std::string_view additionals, Iter iter, ElemPrinter&& elemPrinter)
             {ostream << additionals; elemPrinter( ostream, *(std::begin(iter)) );}
    void printIterableOnStream( std::ostream& ostream, const Iter &iter,
