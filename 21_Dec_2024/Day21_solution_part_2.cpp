...
NAoc__MR::TResult day21Part2(std::shared_ptr<std::istream> inputStream)
{
   ...
   using BestSeqsForDir = std::vector< std::vector< TResult > >;

   const auto NumRobotDirPads = IsShortTest ? 2U : 25U;
   std::vector< BestSeqsForDir > bestSeqsByLevel(NumRobotDirPads, BestSeqsForDir{FourDir4+1, std::vector(FourDir4+1,TResult{0U})});
   ...
   if (NumRobotDirPads > 0U){
      ...
      for(int i = 0; i <= FourDir4; ++i){
         for(int j = 0; j <= FourDir4; ++j){
            ...
            bestSeqsForDir[i][j] = std::min(seq1.size(),seq2.size());
         }
      }
   }
   ...
   const auto generateBestSeqsBasedOnPrevious =
      ...
      for(int i = 0; i <= FourDir4; ++i){
         for(int j = 0; j <= FourDir4; ++j){
            ...
            TResult l1 = (levelIdx+1U < bestSeqsByLevel.size()) // NumRobotDirPads
                         ? generateBestSeqFromSequence(bestSeqsByLevel[levelIdx+1U],seqSource1)
                         : seqSource1.size();
            TResult l2 = (levelIdx+1U < bestSeqsByLevel.size()) // NumRobotDirPads
                         ? generateBestSeqFromSequence(bestSeqsByLevel[levelIdx+1U],seqSource2)
                         : seqSource2.size();

            bestSeqsForDir[i][j] = std::min(l1,l2);
         }
      }
   };
   ...
   for(const auto& code : lines){
      ...
      for(char ch : code){
         ...
         TResult l1 = (NumRobotDirPads > 0U)
                      ? generateBestSeqFromSequence(bestSeqsByLevel[0],seqSource1)
                      : seqSource1.size();
         TResult l2 = (NumRobotDirPads > 0U)
      ...
      if (codeVal[0] > std::numeric_limits<TResult>::max()/l){
         throw std::runtime_error("TResult to be wider");
      }
      auto add = (codeVal[0] * l);
