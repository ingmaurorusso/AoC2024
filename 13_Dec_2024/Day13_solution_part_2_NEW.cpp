...
NAoc__MR::TResult day13Part2(std::shared_ptr<std::istream> inputStream)
{
   ...
   const auto checkPressNumber = [](PointD p){
      ...
      if (std::abs(det) > 0.5){
         auto solSys = inv * prize;

         std::cout << (inv*matrix) << std::endl;

         pInters.x = solSys[0][0];
         pInters.y = solSys[0][1];

         auto pI2 = pInters;
         pInters.x = (matrix[1][1] * prizeRow[0] - matrix[0][1] * prizeRow[1]) / det;
         pInters.y = (matrix[0][0] * prizeRow[1] - matrix[1][0] * prizeRow[0]) / det;
         if (std::fabs(pI2.x-pInters.x) + std::fabs(pI2.y-pInters.y) > 0.01){
            pI2 *= 1;
         }
         ...
         if ( (pInters.x >= 0) && (pInters.y >= 0) &&
              (modfl(pInters.x,&integerPart) < ROUND) && (modfl(pInters.y,&integerPart) < ROUND) ){
         ...
         TCoordD det1 = prizeRow[0]*matrix[1][1] - prizeRow[1]*matrix[0][1];
         TCoordD det2 = matrix[0][0]*prizeRow[1] - matrix[1][0]*prizeRow[0];
